...
object AiServiceSync {
    ...
    fun processInlineWithModel(
        ...
        if (steps.size > 1) {
            ...
            for (step in steps) {
                ...
                intermediate = if (attachments.isEmpty()) {
                    callWithModel(stepPrompt, aiModel, prefs, cancelHandle).trim()
                } else {
                    processWithModelAndInstructionWithAttachments(
                        text = intermediate,
                        aiModel = aiModel,
                        instruction = resolved + chainSuffix,
                        prefs = prefs,
        ...
        return if (attachments.isEmpty()) {
            ...
            callWithModel(prompt, aiModel, prefs, cancelHandle)
        } else {
            processWithModelAndInstructionWithAttachments(
                text = content,
                aiModel = aiModel,
    ...
    fun processWithModelAndInstruction(
        ...
        prefs: SharedPreferences,
        cancelHandle: AiCancelRegistry.CancelHandle? = null
    ): String = processWithModelAndInstructionWithAttachments(
        text = text,
        aiModel = aiModel,
    ...
    @JvmStatic
    @JvmOverloads
    fun processWithModelAndInstructionWithAttachments(
        text: String,
        aiModel: String,
        instruction: String,
        prefs: SharedPreferences,
        ...
        if (attachments.isNotEmpty() && supportsImageInput(aiModel)) {
            val messages = listOf(ChatMessage(role = "user", content = prompt, attachments = attachments))
            return chatCompletionMultiTurn(messages, aiModel, prefs, cancelHandle)
        }
    ...
    fun isVisionCapable(modelValue: String): Boolean {
        ...
        if (modelValue.startsWith("openai-cloud:")) return true
        if (modelValue.startsWith("anthropic:")) return true
        if (modelValue.startsWith("gemini:")) return true
        if (modelValue.startsWith("groq:")) return true
        if (modelValue.startsWith("openrouter:")) return true
        return false
    }
    ...
    data class LatestImageCandidate(
        @JvmField val uriString: String,
        @JvmField val mimeType: String
    )

    @JvmStatic
    fun shouldOfferLatestImageForModel(modelValue: String): Boolean {
        if (!supportsImageInput(modelValue)) return false
        return modelValue.startsWith("openrouter:") ||
            modelValue.startsWith("groq:") ||
            modelValue.startsWith("gemini:") ||
            modelValue.startsWith("anthropic:") ||
            modelValue.startsWith("openai-cloud:")
    }

    @JvmStatic
    fun findLatestImageCandidate(context: Context): LatestImageCandidate? {
        ...
        val selection = if (android.os.Build.VERSION.SDK_INT >= 29) {
            "${android.provider.MediaStore.Images.Media.IS_PENDING} = 0"
        } else {
            null
        }
        ...
        return try {
            context.contentResolver.query(
                ...
                while (cursor.moveToNext()) {
                    ...
                    val mime = cursor.getString(1)?.takeIf { it.startsWith("image/") } ?: "image/jpeg"
                    val dateAdded = cursor.getLong(2) // in seconds
                    
                    val nowSeconds = System.currentTimeMillis() / 1000
                    if (nowSeconds - dateAdded > 3600) {
                        // Image is older than 1 hour, skip it. 
                        // Since we sort DESC, all subsequent images are also old.
                        break
                    }
                    ...
                    // Check if it's recently added (within last 5 minutes)
                    // or just return the first one found if we want "any" latest.
                    // The user specifically asked for "recently taken screenshot".
                    return LatestImageCandidate(uri.toString(), mime)
                }
                null
            }
        } catch (_: Exception) {
    ...
    @JvmStatic
    fun cacheImageAttachmentFromUri(
        context: Context,
        uriString: String,
        mimeType: String = "image/jpeg"
    ): MessageAttachment? {
        val uri = try { android.net.Uri.parse(uriString) } catch (_: Exception) { return null }
        ...
        return try {
            val outDir = java.io.File(context.cacheDir, "ai_latest_image").apply { mkdirs() }
            val outFile = java.io.File(outDir, "latest_${System.currentTimeMillis()}.$ext")
            context.contentResolver.openInputStream(uri)?.use { input ->
                java.io.FileOutputStream(outFile).use { output -> input.copyTo(output) }
            } ?: return null
            MessageAttachment(outFile.absolutePath, mime)
        } catch (_: Exception) {
            null
        }
    }
