...
public final class InputLogic {
    ...
    public InputTransaction onTextInput(final SettingsValues settingsValues, final Event event,
        ...
        // Redirect to active dialog views if needed
        final android.widget.EditText dialogEdit = mLatinIME.getDialogEditText();
        final android.webkit.WebView dialogWebView = mLatinIME.getFocusedWebView();
        if (dialogEdit != null || dialogWebView != null) {
            if (dialogEdit != null) {
                ...
                dialogEdit.getText().replace(start, end, rawText);
                mLatinIME.setDialogCursorPos(start + rawText.length());
            } else {
                handleWebViewInput(dialogWebView, rawText);
            }
            return new InputTransaction(settingsValues, event, SystemClock.uptimeMillis(), mSpaceState, getActualCapsMode(settingsValues, keyboardShiftMode));
        }
    ...
    public InputTransaction onCodeInput(final SettingsValues settingsValues,
        ...
        // Route key events to dialog views if one is active (IME can't type into its own dialogs normally)
        final android.widget.EditText dialogEdit = mLatinIME.getDialogEditText();
        final android.webkit.WebView dialogWebView = mLatinIME.getFocusedWebView();
        if (dialogEdit != null || dialogWebView != null) {
            ...
            if (dialogEdit != null) {
                ...
                } else if (keyCode == KeyCode.CLIPBOARD_COPY) {
                    dialogEdit.onTextContextMenuItem(android.R.id.copy);
                } else if (keyCode == KeyCode.CLIPBOARD_PASTE) {
                    dialogEdit.onTextContextMenuItem(android.R.id.paste);
                } else if (keyCode == KeyCode.CLIPBOARD_CUT) {
                ...
                handleWebViewInput(dialogWebView, code, keyCode);
            }
            return inputTransaction;
        }
    ...
    private void handleFunctionalEvent(final Event event, final InputTransaction inputTransaction,
        ...
        switch (keyCode) {
            ...
            case KeyCode.FLOATING_NOTE:
                mLatinIME.showFloatingNoteTool();
                break;
    ...
    private interface LatestImageDecisionCallback {
        void onDecision(@Nullable helium314.keyboard.latin.ai.AiServiceSync.MessageAttachment attachment);
    }

    private void maybeAttachLatestImageWithConfirmation(
            final String modelSpec,
            final LatestImageDecisionCallback callback
    ) {
        final android.content.SharedPreferences prefs =
            helium314.keyboard.latin.utils.DeviceProtectedUtils.getSharedPreferences(mLatinIME);
        final boolean enabled = prefs.getBoolean(
            Settings.PREF_AI_ASK_ATTACH_LATEST_IMAGE,
            helium314.keyboard.latin.settings.Defaults.PREF_AI_ASK_ATTACH_LATEST_IMAGE
        );
        if (!enabled || !helium314.keyboard.latin.ai.AiServiceSync.shouldOfferLatestImageForModel(modelSpec)) {
            callback.onDecision(null);
            return;
        }
        ...
        if (android.os.Build.VERSION.SDK_INT >= 34) {
            hasPermission = androidx.core.content.ContextCompat.checkSelfPermission(mLatinIME, android.Manifest.permission.READ_MEDIA_IMAGES)
                == android.content.pm.PackageManager.PERMISSION_GRANTED ||
                androidx.core.content.ContextCompat.checkSelfPermission(mLatinIME, "android.permission.READ_MEDIA_VISUAL_USER_SELECTED")
                == android.content.pm.PackageManager.PERMISSION_GRANTED;
        } else if (android.os.Build.VERSION.SDK_INT >= 33) {
            hasPermission = androidx.core.content.ContextCompat.checkSelfPermission(mLatinIME, android.Manifest.permission.READ_MEDIA_IMAGES)
                == android.content.pm.PackageManager.PERMISSION_GRANTED;
        } else {
            hasPermission = androidx.core.content.ContextCompat.checkSelfPermission(mLatinIME, android.Manifest.permission.READ_EXTERNAL_STORAGE)
                == android.content.pm.PackageManager.PERMISSION_GRANTED;
        }
        if (!hasPermission) {
            showAiBriefMessage("Storage permission required for latest image context", 2500);
            callback.onDecision(null);
            return;
        }

        final boolean alwaysShowGallery = prefs.getBoolean(
            Settings.PREF_AI_ALWAYS_SHOW_GALLERY_PROMPT,
            helium314.keyboard.latin.settings.Defaults.PREF_AI_ALWAYS_SHOW_GALLERY_PROMPT
        );

        if (alwaysShowGallery) {
            startGalleryPicker(modelSpec, callback);
            return;
        }

        final helium314.keyboard.latin.ai.AiServiceSync.LatestImageCandidate candidate =
            helium314.keyboard.latin.ai.AiServiceSync.findLatestImageCandidate(mLatinIME);
        if (candidate == null) {
            showAiBriefMessage("No recent image found", 1500);
            callback.onDecision(null);
            return;
        }

        showImageConfirmationDialog(candidate, modelSpec, callback);
    }

    private static LatestImageDecisionCallback sPendingImageCallback;
    private static String sPendingModelSpec;

    private void startGalleryPicker(String modelSpec, LatestImageDecisionCallback callback) {
        sPendingImageCallback = callback;
        sPendingModelSpec = modelSpec;
        android.content.Intent intent = new android.content.Intent(mLatinIME, helium314.keyboard.latin.ai.ImagePickerTrampolineActivity.class);
        intent.addFlags(android.content.Intent.FLAG_ACTIVITY_NEW_TASK);
        mLatinIME.startActivity(intent);
    }

    public static void onImagePicked(android.net.Uri uri) {
        if (sPendingImageCallback == null || sPendingModelSpec == null) return;
        final LatestImageDecisionCallback callback = sPendingImageCallback;
        final String modelSpec = sPendingModelSpec;
        sPendingImageCallback = null;
        sPendingModelSpec = null;
        ...
        final helium314.keyboard.latin.ai.AiServiceSync.LatestImageCandidate candidate =
            new helium314.keyboard.latin.ai.AiServiceSync.LatestImageCandidate(uri.toString(), "image/jpeg");
        
        helium314.keyboard.latin.LatinIME ime = helium314.keyboard.latin.LatinIME.getInstance();
        if (ime != null) {
            ime.mHandler.post(() -> {
                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.P) {
                    ime.requestShowSelf(0);
                }
                ime.getInputLogic().showImageConfirmationDialog(candidate, modelSpec, callback);
            });
        } else {
            callback.onDecision(null);
        }
    ...
    private void showImageConfirmationDialog(
        ...
        mLatinIME.mHandler.post(() -> {
            final android.content.Context themed = helium314.keyboard.latin.utils.DialogUtilsKt.getPlatformDialogThemeContext(mLatinIME);
            final android.widget.LinearLayout layout = new android.widget.LinearLayout(themed);
            layout.setOrientation(android.widget.LinearLayout.VERTICAL);
            int pad = (int) (16 * themed.getResources().getDisplayMetrics().density);
            layout.setPadding(pad, pad, pad, pad);

            final android.widget.TextView prompt = new android.widget.TextView(themed);
            prompt.setText("Add your latest image as context?");
            prompt.setPadding(0, 0, 0, pad / 2);
            layout.addView(prompt);

            final android.widget.ImageView preview = new android.widget.ImageView(themed);
            preview.setAdjustViewBounds(true);
            preview.setMaxHeight((int) (220 * themed.getResources().getDisplayMetrics().density));
            preview.setImageURI(android.net.Uri.parse(candidate.uriString));
            layout.addView(preview);

            final android.widget.ProgressBar progress = new android.widget.ProgressBar(themed);
            ...
            final android.app.AlertDialog dialog = new android.app.AlertDialog.Builder(themed)
                .setTitle("Latest image")
                .setView(layout)
                .setNegativeButton("Skip", (d, w) -> callback.onDecision(null))
                .setPositiveButton("Add", null)
                .setOnCancelListener(d -> callback.onDecision(null))
                .create();

            final android.view.Window window = dialog.getWindow();
            if (window != null) {
                final android.view.WindowManager.LayoutParams lpWindow = window.getAttributes();
                lpWindow.type = android.view.WindowManager.LayoutParams.TYPE_INPUT_METHOD;
                // Use a token that is likely valid
                android.os.IBinder token = null;
                if (mLatinIME.getWindow() != null && mLatinIME.getWindow().getWindow() != null) {
                    token = mLatinIME.getWindow().getWindow().getAttributes().token;
                }
                lpWindow.token = token;
                window.setAttributes(lpWindow);
            }

            dialog.setOnShowListener(d -> {
                android.widget.Button addBtn = dialog.getButton(android.app.AlertDialog.BUTTON_POSITIVE);
                if (addBtn == null) return;
                addBtn.setOnClickListener(v -> {
                    addBtn.setEnabled(false);
                    android.widget.Button skipBtn = dialog.getButton(android.app.AlertDialog.BUTTON_NEGATIVE);
                    if (skipBtn != null) skipBtn.setEnabled(false);
                    progress.setVisibility(android.view.View.VISIBLE);
                    new Thread(() -> {
                        final helium314.keyboard.latin.ai.AiServiceSync.MessageAttachment attachment =
                            helium314.keyboard.latin.ai.AiServiceSync.cacheImageAttachmentFromUri(
                                mLatinIME,
                                candidate.uriString,
                                candidate.mimeType
                            );
                        mLatinIME.mHandler.post(() -> {
                            ...
                            callback.onDecision(attachment);
                        });
                    }).start();
                });
            });
    ...
    private void handleAiAssist() {
        ...
        maybeAttachLatestImageWithConfirmation(aiModel, attachment -> {
            mAiProcessing = true;
            ...
            final java.util.List<helium314.keyboard.latin.ai.AiServiceSync.MessageAttachment> attachments =
                attachment == null ? java.util.Collections.emptyList() : java.util.Collections.singletonList(attachment);

            new Thread(() -> {
                try {
                    final String result;
                    if (fInlineInstruction != null) {
                        result = helium314.keyboard.latin.ai.AiServiceSync.processInlineWithModel(
                            fInlineContent, fInlineInstruction, aiModel, prefs, handle, attachments
                        );
                    } else {
                        result = helium314.keyboard.latin.ai.AiServiceSync.processWithModelAndInstructionWithAttachments(
                            text,
                            aiModel,
                            prefs.getString(Settings.PREF_AI_INSTRUCTION, helium314.keyboard.latin.settings.Defaults.PREF_AI_INSTRUCTION),
                            prefs,
                            attachments,
                            handle
                        );
                            ...
                            () -> {
                                ...
                                new Thread(() -> {
                                    try {
                                        ...
                                        if (fInlineInstruction != null) {
                                            retryResult = helium314.keyboard.latin.ai.AiServiceSync.processInlineWithModel(
                                                fInlineContent, fInlineInstruction, aiModel, prefs, retryHandle, attachments
                                            );
                                        } else {
                                            retryResult = helium314.keyboard.latin.ai.AiServiceSync.processWithModelAndInstructionWithAttachments(
                                                text,
                                                aiModel,
                                                prefs.getString(Settings.PREF_AI_INSTRUCTION, helium314.keyboard.latin.settings.Defaults.PREF_AI_INSTRUCTION),
                                                prefs,
                                                ...
                                                () -> {
                                                    ...
                                                    new Thread(() -> {
                                                        try {
                                                            final String rr2;
                                                            if (fInlineInstruction != null) {
                                                                rr2 = helium314.keyboard.latin.ai.AiServiceSync.processInlineWithModel(
                                                                    fInlineContent, fInlineInstruction, aiModel, prefs, rh2, attachments
                                                                );
                                                            } else {
                                                                rr2 = helium314.keyboard.latin.ai.AiServiceSync.processWithModelAndInstructionWithAttachments(
                                                                    text,
                                                                    aiModel,
                                                                    prefs.getString(Settings.PREF_AI_INSTRUCTION, helium314.keyboard.latin.settings.Defaults.PREF_AI_INSTRUCTION),
                                                                    prefs,
    ...
    private void handleAiSlot(int slotNumber) {
        ...
        final String modelForImageOffer = modelSpecRaw;
        maybeAttachLatestImageWithConfirmation(modelForImageOffer, attachment -> {
            mAiProcessing = true;
            ...
            new Thread(() -> {
                try {
                    ...
                    if (slotInstruction != null && !slotInstruction.isEmpty()) {
                        result = helium314.keyboard.latin.ai.AiServiceSync.processWithModelAndInstructionWithAttachments(
                            text, modelSpec, slotInstruction, prefs, attachments, handle
                        );
                    } else {
                        result = helium314.keyboard.latin.ai.AiServiceSync.processWithModelAndInstructionWithAttachments(
                            text,
    ...
    private void handleWebViewInput(android.webkit.WebView wv, int code, int keyCode) {
        if (keyCode == KeyCode.DELETE) {
            wv.dispatchKeyEvent(new android.view.KeyEvent(android.view.KeyEvent.ACTION_DOWN, android.view.KeyEvent.KEYCODE_DEL));
            wv.dispatchKeyEvent(new android.view.KeyEvent(android.view.KeyEvent.ACTION_UP, android.view.KeyEvent.KEYCODE_DEL));
        } else if (keyCode == Constants.CODE_ENTER) {
            // More aggressive Enter simulation
            String script = "(function() { " +
                ...
                "  if (el.tagName === 'INPUT' && el.type === 'search') el.form && el.form.submit(); " +
                "} " +
                "})();";
            wv.evaluateJavascript(script, null);
        } else if (keyCode == KeyCode.CLIPBOARD_COPY) {
            wv.evaluateJavascript("document.execCommand('copy');", null);
        } else if (keyCode == KeyCode.CLIPBOARD_PASTE) {
            handleWebViewInput(wv, mLatinIME.getClipboardHistoryManager().retrieveClipboardContent().toString());
        } else if (keyCode == KeyCode.CLIPBOARD_CUT) {
            wv.evaluateJavascript("document.execCommand('cut');", null);
        } else if (keyCode == KeyCode.CLIPBOARD_SELECT_ALL) {
            wv.evaluateJavascript("document.execCommand('selectAll');", null);
        } else if (keyCode == KeyCode.ARROW_LEFT) {
            wv.dispatchKeyEvent(new android.view.KeyEvent(android.view.KeyEvent.ACTION_DOWN, android.view.KeyEvent.KEYCODE_DPAD_LEFT));
            wv.dispatchKeyEvent(new android.view.KeyEvent(android.view.KeyEvent.ACTION_UP, android.view.KeyEvent.KEYCODE_DPAD_LEFT));
        } else if (keyCode == KeyCode.ARROW_RIGHT) {
            wv.dispatchKeyEvent(new android.view.KeyEvent(android.view.KeyEvent.ACTION_DOWN, android.view.KeyEvent.KEYCODE_DPAD_RIGHT));
            wv.dispatchKeyEvent(new android.view.KeyEvent(android.view.KeyEvent.ACTION_UP, android.view.KeyEvent.KEYCODE_DPAD_RIGHT));
        } else if (keyCode == KeyCode.ARROW_UP) {
            wv.dispatchKeyEvent(new android.view.KeyEvent(android.view.KeyEvent.ACTION_DOWN, android.view.KeyEvent.KEYCODE_DPAD_UP));
            wv.dispatchKeyEvent(new android.view.KeyEvent(android.view.KeyEvent.ACTION_UP, android.view.KeyEvent.KEYCODE_DPAD_UP));
        } else if (keyCode == KeyCode.ARROW_DOWN) {
            wv.dispatchKeyEvent(new android.view.KeyEvent(android.view.KeyEvent.ACTION_DOWN, android.view.KeyEvent.KEYCODE_DPAD_DOWN));
            wv.dispatchKeyEvent(new android.view.KeyEvent(android.view.KeyEvent.ACTION_UP, android.view.KeyEvent.KEYCODE_DPAD_DOWN));
        } else if (code > 0) {
            handleWebViewInput(wv, String.valueOf((char) code));
        }
    }

    private void handleWebViewInput(android.webkit.WebView wv, String text) {
        String escaped = text.replace("\\", "\\\\").replace("'", "\\'").replace("\n", "\\n").replace("\r", "\\r");
        String script = "document.execCommand('insertText', false, '" + escaped + "');";
        wv.evaluateJavascript(script, null);
    }
}
